...
def test_post_item_types_integration(_data):
    ...
    get_response = requests.get(f"{url}/{typeId}", headers=headers)
    ...
    # Cleanup by deleting the created item type
    requests.delete(f"{url}/{typeId}", headers=headers)
...
def test_put_item_types_integration(_data):
    ...
    # Get the current item type data
...
#     # Verify that the item type is deleted
...
#     # Verify that the status code is 404 (Not Found) and the item type no longer exists
