...
def test_get_suppliers_by_id_integration(_data):
    url = _data[0]["URL"] + 'suppliers/11'
    # params = {'id': 12}
    header = _data[1]
    ...
    # Verify that the status code is 200 (OK)
    assert status_code == 200 and response_data["id"] == 11
...
def test_put_suppliers_integration(_data):
    ...
    assert status_code == 200 and response_data["id"] == body["id"] and response_data["name"] == body["name"] and response_data["reference"] == body["reference"]


def test_delete_suppliers_integration(_data):
    url = _data[0]["URL"] + 'suppliers/8'
    header = _data[1]
